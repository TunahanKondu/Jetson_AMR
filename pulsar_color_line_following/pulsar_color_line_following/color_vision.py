...
def detect_orange_line(
    frame, roi_start_ratio=0.45, roi_end_ratio=0.88,
    hsv_lower=(5, 80, 70), hsv_upper=(25, 255, 255),
    min_area_ratio=0.0005, minimum_height_ratio=0.18,
    previous_center_x=None, maximum_center_jump_ratio=0.25,
):
    ...
    if not 0 <= roi_start_ratio < roi_end_ratio <= 1:
        raise ValueError('ROI ratios must satisfy 0 <= start < end <= 1')
    top = int(height * roi_start_ratio)
    bottom = max(top + 1, int(height * roi_end_ratio))
    ...
    if lower.shape != (3,) or upper.shape != (3,) or np.any(lower > upper):
        raise ValueError('HSV bounds must be three increasing channel values')
    ...
    candidates = []
    minimum_area = float(min_area_ratio) * width * (bottom - top)
    for contour in contours:
        area = cv2.contourArea(contour)
        if area < minimum_area:
            continue
        _, _, stripe_width, stripe_height = cv2.boundingRect(contour)
        if stripe_height < minimum_height_ratio * (bottom - top):
            continue
        ...
        y = int(moments['m01'] / moments['m00']) + top
        if previous_center_x is not None:
            distance = abs(x - previous_center_x)
            if distance > maximum_center_jump_ratio * width:
                continue
            reference = previous_center_x
        else:
            reference = width / 2
        score = (area / max(stripe_width * stripe_height, 1)
                 - 0.35 * abs(x - reference) / width)
        candidates.append((score, contour, (x, y), area))
    if not candidates:
        return ColorDetection(mask, top, bottom)
    _, contour, center, area = max(candidates, key=lambda item: item[0])
    error = float(np.clip((center[0] - width / 2) / (width / 2), -1, 1))
    return ColorDetection(mask, top, bottom, contour, center, error, area)
